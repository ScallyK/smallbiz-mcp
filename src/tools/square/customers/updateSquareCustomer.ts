...
export default function updateSquareCustomer(mcpServerName: McpServer) {

  mcpServerName.registerTool(
    "update-square-customer",
    {
      title: "Update Square Customer",
      description: "Update a customer in Square for a given customer ID.",
      inputSchema: {
        customerId: z.string(),
        address: z.object({
    ...
    async ({
      ...
      try {
        ...
        const updatedCustomer = await squareClient.customers.update({
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error updating Square customer: ${errorMessage}`,
            },
          ],
        };
      }
